...
class ExtruderControlEnvFixed(gym.Env):
    '''
    ### Description
    ...
    ### Action Space
    We are planning to use a 1-dimensional discrete space as the action space, the acceleration required to achieve
    that speed. Acceleration ranges from -1 to 1, step is 0.1 (normalized), where a value less than 0 means deceleration, a value equal to 0
    means maintaining the current speed, and a value greater than 0 means acceleration.


    ### Observation Space
    We use discrete space as Observation Space. Typically, 100 selected checkpoints on the printing tool path. The distance
    between two checkpoints is 35mm now. Each sate is a 6-dimensional vector: current checkpoint, current speed and current acceleration,
    ...
    **Note**: We decide to use 30 mm/s as the first layer speed, the first layer time would be 117s (59 frame).
    ...
    def __init__(self, env_config=None):
        super(ExtruderControlEnvFixed, self).__init__()
